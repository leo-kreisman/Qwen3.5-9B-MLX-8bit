...
REPO="${REPO:-leo-kreisman/Qwen3.5-9B-MLX-8bit}"
TAG="${TAG:-weights-v1}"
NAME="${NAME:-Weights (split into <2 GiB parts)}"
PARTS_DIR=""
DRY_RUN=0
...
if [ -z "$release_id" ] && [ "$DRY_RUN" -eq 0 ]; then
  ...
  body="Weight shards for mlx-community/Qwen3.5-9B-MLX-8bit, split into <2 GiB parts.

Each \`*.safetensors\` is cut into three parts because GitHub caps files at
2 GiB. Concatenate \`part-0 part-1 part-2\` in order to reproduce the original
file; \`MANIFEST.sha256\` lists the sha256 of every part and of each
reassembled shard. Run \`./assemble.sh\` in the repository to do this
automatically with verification.

Mirror of https://huggingface.co/mlx-community/Qwen3.5-9B-MLX-8bit (Apache-2.0)."
